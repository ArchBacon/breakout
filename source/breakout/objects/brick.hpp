...
inline void GetBricksFromStage(const int stage, const breakout::Bounds& fieldBounds, std::vector<std::unique_ptr<Brick>>& bricks, int& bricksToClear)
{
    // Reset return values
    bricks.clear();
    bricksToClear = 0;
    ...
    Engine.FileIO().ReadFilePerLine("assets/stages/stage_" + ZeroFill(stage, 2) + ".txt", [&](const std::string& line, const unsigned lineNumber)
    {
        ...
        for (int x = 0; x < static_cast<int>(line.size()); x++)
        {
            ...
            if (color == Color::Silver)
            {
                ...
                bricks.push_back(std::unique_ptr<Brick>(brick));
                bricksToClear++;
                continue;
            }
